...
// Generate URL-safe tokens using nanoid (Edge Runtime compatible)
const nanoid = customAlphabet('0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz-_');
...
export async function claimGenericInvite(
  ...
  // If email already exists in another row (claimed but not accepted), delete that row first
  // This prevents the unique constraint violation when we update the current invite
